...
void VKE::RenderContext::init(GLFWwindow* window) {

	glfw_window_ = window;

	createInstance();
	setupDebugMessenger();
	createSurface(glfw_window_);
	pickPhysicalDevice();
	createLogicalDevice();
	createSwapChain();
	createImageViews();
...
void VKE::RenderContext::createInstance() {
	...
	createInfo.enabledExtensionCount = extensions_vec.size();
	...
	std::function <std::string(const char[256])> checkIfIncluded =

		[glfwExtensionCount, glfwExtensions](const char ext_name[256]) {
...
bool VKE::RenderContext::isDeviceSuitable(VkPhysicalDevice device) {
	...
	return discrete_gpu && indices.isComplete() && extensionsSupported && swapChainAdequate;
...
std::vector<const char*> VKE::RenderContext::getRequiredExtensions() {
